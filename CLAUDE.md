...
- 预计运行时间超过 90 秒、且用户需要完成汇报时，使用 `bridge-cli bg enqueue`。
- 短命令、需要即时交互的命令，直接在当前 runner 执行，不放入 bg task。
- 完全不需要回报给会话的维护命令，用普通 fire-and-forget 机制；不要伪造 `on_done_prompt`。
- bg task 命令必须用 argv：`bridge-cli bg enqueue --chat-id oc_xxx --on-done-prompt "done" -- sleep 10`。
...
- 取消任务用 `bridge-cli bg cancel <task_id>`，不要直接 kill 用户进程，除非是在调试 bg reconciler 本身。
