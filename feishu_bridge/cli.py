...
def _parse_env_kv(s: str) -> tuple[str, str]:
    # Error messages deliberately avoid echoing the raw value — users may
    # paste secrets with a typo (missing `=`), and stderr can end up in
    # shell history / logs.
    if "=" not in s:
        raise ValueError(
...
def _run_bg_command(args) -> None:
    ...
    def _ensure_db() -> None:
        """Create ~/.feishu-bridge/ + init schema. Call only after arg
        validation so invalid invocations don't leave on-disk state."""
        _bg_ensure_home()
        try:
    ...
    if sub == "enqueue":
        # argv source: either trailing positional after `--` or --cmd-json,
        # exactly one. Reject bare string (empty positional + no --cmd-json).
        ...
        # All args validated — now it's safe to create on-disk state.
        _ensure_db()
    ...
    # Read paths require the DB to already exist; missing file is not an
    # error we want to swallow silently — surface it as a consistent JSON error.
    ...
    if sub == "cancel":
        ...
        try:
            try:
                ...
                if TaskState.is_terminal(row.state):
                    ...
                    sys.exit(1)
                # set_cancel_requested excludes terminal states via WHERE clause;
                # a False return means the row transitioned between get() and
                # UPDATE (TOCTOU). Surface that honestly instead of claiming success.
        ...
        try:
            ...
            # Non-UUID task_id: still succeed on DB update, skip targeted nudge
            _bg_nudge(sock_path, b"\x01")
...
def main():
    ...
    # --- Background task commands (no Feishu auth needed) ---
    if args.command == "bg":
