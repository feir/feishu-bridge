...
### 会话与上下文管理
跨消息维持完整上下文，bridge 重启自动恢复会话。Context 用量实时监控（70%/85% 阈值告警），支持 `/compact` 压缩。空闲会话在 prompt cache TTL 到期前自动触发 compact（idle auto-compact），降低冷启动成本。每个聊天独立任务队列（单聊串行、多聊天 4 workers 并行），`/btw` 可在不中断当前任务的情况下侧问（仅 Claude）。

### 交互增强
- **Action buttons** — Claude 需要确认/选择时，卡片内直接渲染交互按钮，点击即响应
- **内联 URL 按钮** — 回复中的链接自动提取为侧边栏快捷按钮
- **Rich footer** — 每张卡片底部显示状态、模型、耗时、token、项目（`/project` 绑定）、context 告警、版本更新

### 飞书 API 集成

**Bridge 内置 Wrappers** — 6 个飞书服务的 Python wrapper，自动管理 OAuth 令牌（AES-256-GCM 加密缓存），权限不足时在会话内弹出授权卡片而非退出到浏览器：

| 服务 | 文件 | 操作数 |
|------|------|--------|
| Tasks | `api/tasks.py` | 14（CRUD + 清单管理） |
| Docs | `api/docs.py` | 4（Markdown 读写） |
| Sheets | `api/sheets.py` | 6（单元格读写 + 表格 CRUD） |
| Bitable | `api/bitable.py` | 20（表/字段/记录/视图 CRUD） |
| Calendar | `api/calendar.py` | 9（日程查询/创建/更新/删除） |
| Mail | `api/mail.py` | 11（邮件摘要/全文/草稿/回复/发送） |

Agent 通过 bridge **Control API**（Unix socket JSON-RPC）的 `call_service` 方法调用这些 wrapper，无需各自管理 OAuth 令牌。授权卡片直接在飞书会话中弹出，一键完成。

**消息发送** — `feishu-cli` 负责 bot 消息发送（文本、图片、音频）。

**lark CLI** — [larksuite/cli](https://github.com/larksuite/cli) 覆盖 20+ 飞书服务，可作为独立 CLI 工具或 bridge wrapper 之外的补充方案。

### 多 Agent 后端

| 特性 | Claude Code | Codex | OMP | Pi |
|------|-------------|-------|-----|-----|
| 流式输出 | 增量实时更新 | 流式输出 | 增量实时更新 | 增量实时更新 |
| `/compact` | ✅ | ❌ | ✅ | ❌ |
| `/btw` | ✅ | ❌ | ❌ | ❌ |
| `/status` | ✅ | ✅ | ✅ | ✅ |
...
| `/btw <问题>` | 侧问（fork 会话，不中断当前任务；仅 Claude） |
| `/compact [指令]` | 压缩上下文（仅 Claude / OMP） |
| `/model [模型名]` | 查看或切换模型 |
| `/agent [类型]` | 查看或切换后端（claude / codex / pi / omp） |
| `/provider [名称]` | 查看或切换当前后端配置 |
| `/status` | 查看会话状态（context / 费用 / 配额） |
| `/update` | 拉取最新版本并重启（若已是最新则仅提示） |
| `/plan [目标]` | 生成实施计划草稿（bridge 工作流） |
| `/done [摘要]` | 归档当前会话（bridge 工作流） |
...
> **工作流命令**（`/plan` `/done` `/memory-gc`）由 bridge 内置工作流引擎执行，不经过 Agent CLI。需要在 `~/.agents/skills/` 下安装对应 skill。工作流产出草稿后需 `/confirm` 确认，或 `/stop` 取消。
...
```jsonc
{
  "bots": [{
    ...
    "model": "claude-opus-4-6",         // 可选：透传给 CLI；未设则由 CLI 决定
    "group_policy": { ... }             // 可选：群聊响应策略
  }],
  "agent": {
    "type": "claude",                   // claude | codex | pi | omp
    "command": "claude",
...
发送 `/agent claude`、`/agent codex`、`/agent pi`、`/agent omp` 可在当前 bot 进程内热切换后端。切换会清空旧会话映射，避免复用不兼容的 `session_id` / `thread_id`；该切换仅影响当前运行实例，重启后仍以配置文件中的 `agent.type` 为准。
...
### 从 `type=local` 迁移（2026-04-19 起）

LocalHTTPRunner 已于 2026-04-19 移除。之前直接对接 OpenAI-compatible 端点的 `agent.type=local` 配置会在启动时 `sys.exit(1)` 并提示迁移路径。

替换方案：改用 `agent.type=codex` + Codex OSS 本地 provider（`--oss` 等价于 `-c model_provider=oss`），由 Codex CLI 自己管理 Ollama / LM Studio 等本地端点。

| 旧配置（type=local） | 新配置（type=codex + Codex OSS） |
|----------------------|-----------------------------------|
| `"type": "local"`<br>`"endpoint": "http://127.0.0.1:11434/v1"`<br>`"model": "gpt-oss:120b"` | `"type": "codex"`<br>`"args_by_type": { "codex": ["--oss", "--local-provider", "ollama"] }`<br>`"model": "gpt-oss:120b"`（透传给 Codex CLI） |

迁移后 bridge 不再直连 HTTP 端点，所有请求经 Codex CLI 转发，保留 Codex 的事件流卡片渲染。LM Studio 用户把 `--local-provider` 值改为 `lmstudio` 即可。
...
模型名由 CLI 决定：bridge 只透传 `"model"` 字段（或 `/model` 命令参数）给 CLI，不内置任何默认模型。本地已安装的模型名由 Ollama/oMLX/Pi 等下游 CLI 管理。
...
同样可以通过 bot 配置里的 `"model"` 或 `/model gpt-oss:120b` 指定具体模型（bridge 只透传）。若你已在 `~/.codex/config.toml` 配好 profile，也可以改用：
...
Pi runner 可通过 `feishu-bridge` skill（`~/.pi/agent/skills/feishu-bridge/`）直接调用 bridge 的 6 个 API wrapper，无需走独立 `lark` CLI 的 OAuth 流程。skill 通过 Unix socket 连接 bridge Control API 的 `call_service` 方法，复用会话内授权卡片。

```bash
# Pi 在 bash 中调用
python3 ~/.pi/agent/skills/feishu-bridge/scripts/call.py \
  --service calendar --action agenda --args '{}'
```
...
OMP runner 使用持久化 JSON-RPC 2.0 进程（persistent RPC mode），每个 session 对应一个长驻 `omp` 进程，通过 stdin/stdout 通信。进程跨 turn 复用，crash 后从 session 文件自动恢复。

前置条件：

- `omp` 已在 PATH 中，或在 `agent.commands.omp` 中指定路径。
- `~/.omp/agent/config.yml` 已配置（包含 `modelRoles.default` 等）。
...
OMP 支持 `/compact`、`/status`，不支持 `/btw`。空闲 auto-compact 同样生效。
...
| 下游 | 模型来源 | context window 来源 |
|------|----------|---------------------|
| Claude CLI | `--model` 参数或 CLI 默认 | CLI 自带（Anthropic SDK） |
| Codex CLI | `~/.codex/config.toml` 里的 `model` + profile | Codex 自带 |
| Pi CLI | `~/.pi/models.json`（每模型 `contextWindow`）+ `~/.pi/settings.json` | Pi 自带 |
| OMP | `~/.omp/agent/config.yml` 里的 `modelRoles.default` | OMP 自带 |
