...
`feishu-cli` 负责 bot 消息发送（文本、图片、音频），`lark`（[larksuite/cli](https://github.com/larksuite/cli)）覆盖 20+ 飞书服务（文档、表格、多维表格、Wiki、日历、任务、邮件、云盘、审批等）。Agent 自动按需调用，也可作为独立 CLI 工具使用。

### 多 Agent 后端

| 特性 | Claude Code | Codex | OMP | Alma | Pi |
|------|-------------|-------|-----|------|-----|
| 流式输出 | 增量实时更新 | 流式输出 | 增量实时更新 | 增量实时更新 | 增量实时更新 |
| `/compact` | ✅ | ❌ | ✅ | ✅ | ❌ |
| `/btw` | ✅ | ❌ | ❌ | ❌ | ❌ |
| `/status` | ✅ | ✅ | ✅ | ✅ | ✅ |
| 会话持久化 | session_id | thread_id | RPC 进程 | WS thread_id | session 文件 |
| 空闲 auto-compact | ✅ | ❌ | ✅ | ✅ | ❌ |
| 默认模型 | 由 Claude CLI 决定 | 由 Codex CLI 决定 | `~/.omp/agent/config.yml` | Alma 配置 | `~/.pi/models.json` |
...
| `/compact [指令]` | 压缩上下文（仅 Claude / OMP / Alma） |
| `/model [模型名]` | 查看或切换模型 |
| `/agent [类型]` | 查看或切换后端（claude / codex / pi / omp / alma） |
...
```jsonc
{
  ...
  "agent": {
    "type": "claude",                   // claude | codex | pi | omp | alma
    "command": "claude",
...
Alma runner 通过本地 WebSocket API（`localhost:23001`）与 Alma 桌面应用通信，而非启动子进程。Bridge 保持 Feishu 卡片渲染能力；LLM 调用、工具执行、memory 和 compaction 由 Alma 处理。

前置条件：

- Alma 桌面应用已运行并监听 `localhost:23001`。
...
Alma 支持 `/compact`、`/status`，不支持 `/btw`。模型可通过内置别名切换：`/model opus`、`/model sonnet`、`/model haiku`。
...
| `bridge-cli` | Bridge 管理工具（后台任务等） |

飞书服务 API 操作（文档、表格、日历、任务、邮件等）使用独立的 [lark CLI](https://github.com/larksuite/cli)：
